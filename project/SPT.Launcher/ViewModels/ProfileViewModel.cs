...
namespace SPT.Launcher.ViewModels
{
    ...
    public class ProfileViewModel : ViewModelBase
    {
        ...
        private CancellationTokenSource _autoLaunchCts;
        
        // Свойства для отображения автозапуска
        private bool _isAutoLaunchActive;
        public bool IsAutoLaunchActive
        {
            get => _isAutoLaunchActive;
            set => this.RaiseAndSetIfChanged(ref _isAutoLaunchActive, value);
        }
        
        private int _autoLaunchCountdown;
        public int AutoLaunchCountdown
        {
            get => _autoLaunchCountdown;
            set => this.RaiseAndSetIfChanged(ref _autoLaunchCountdown, value);
        }
        ...
        public ProfileViewModel(IScreen Host, bool AutoLaunch = false) : base(Host)
        {
            ...
            DediAvailabilityText = isDediAvailable ? LocalizationProvider.Instance.dedicated_available : LocalizationProvider.Instance.dedicated_unavailable;
            ...
            {
                _autoLaunchCts = new CancellationTokenSource();
                
                // Проверяем задержку на минимальное значение (3 секунды)
                var delay = LauncherSettingsProvider.Instance.AutoLaunchDelay;
                if (delay < 3)
                {
                    delay = 3;
                }
                
                // Активируем отображение автозапуска
                IsAutoLaunchActive = true;
                AutoLaunchCountdown = delay;
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        if (_autoLaunchCts.IsCancellationRequested) return;
                        
                        await Dispatcher.UIThread.InvokeAsync(() =>
                        {
                            AutoLaunchCountdown = i;
                        });
                        
                        await Task.Delay(1000, _autoLaunchCts.Token);
                    }
                    
                    // Проверяем, не была ли отменена задача
                    if (_autoLaunchCts.IsCancellationRequested) return;
                    
                    // Скрываем индикатор и запускаем игру
                    await Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        IsAutoLaunchActive = false;
                        StartGameCommand();
                    });
                }, _autoLaunchCts.Token);
            }
        }

        /// <summary>
        /// Отменяет автозапуск игры. Должен вызываться при выходе пользователя или деактивации ViewModel.
        /// </summary>
        public void CancelAutoLaunch()
        {
            _autoLaunchCts?.Cancel();
            _autoLaunchCts?.Dispose();
            _autoLaunchCts = null;
            
            // Скрываем индикатор автозапуска
            IsAutoLaunchActive = false;
        ...
        public void CancelAutoLaunchCommand()
        {
            CancelAutoLaunch();
        }
        ...
        {
            // Отменяем автозапуск при выходе
            CancelAutoLaunch();
        ...
        {
            // Отменяем автозапуск, если игра запускается вручную
            CancelAutoLaunch();
        ...
        public async Task UpdateOnlinePlayersCommand()
        {
            ...
            if (players.Length == 0)
            {
                SendNotification("Fika", LocalizationProvider.Instance.no_players_online);
            }
            else
            {
                string playerText = PlayersOnline == 1 ? LocalizationProvider.Instance.player_singular : LocalizationProvider.Instance.players_plural;
                SendNotification("Fika", string.Format(LocalizationProvider.Instance.players_online_count, PlayersOnline, playerText));
            }
        ...
        public async Task UpdateDedicatedStatusCommand()
        {
            ...
            // Show notification
            string statusText = isDediAvailable ? LocalizationProvider.Instance.dedicated_status_available : LocalizationProvider.Instance.dedicated_status_unavailable;
            SendNotification("Fika", statusText);
        }
    }
}
